...
@Service
public class RkJamSerialPortImpl implements RkJamSerialPort {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(RkJamSerialPortImpl.class);
    @Value("${serial_port}")
    ...
    public void sendRequest(byte[] request) {
        ...
        data[0] = RkJamFrameMarker.SOF.getCode();
    ...
    private int putDataInMessage(byte[] bytes, int next) {
        ...
        if (length <= 0 || length <= next || next < 0)
        ...
        if (lastPos >= message.length && checkSumPos < length) {

            if (verifyCheckSum(bytes[checkSumPos])) {
                sendDataToPort(new byte[]{RkJamFrameMarker.ACK.getCode()});
            } else {
                sendDataToPort(new byte[]{RkJamFrameMarker.NAC.getCode()});
            }
    ...
    private synchronized void sortingDataForMessage(byte[] bytes) {
        ...
        while (next < bytes.length) {
            ...
            if (messageCreating) {
                ...
                switch (RkJamFrameMarker.getByCode(bytes[next])) {
