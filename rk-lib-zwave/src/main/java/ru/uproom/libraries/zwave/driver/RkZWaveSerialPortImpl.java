...
public class RkZWaveSerialPortImpl implements RkZWaveSerialPort {
    ...
    RkZWaveDriver serialDataHandler;
    ...
    public void open() {
        ...
        String serial = serialDataHandler.getProperties().getProperty("port_name");
        ...
        try {
            ...
            serialDataHandler.applyPortState(true);
    ...
    private void sendDataToPort(int[] data) {
        ...
        try {
            LOG.debug("REQUEST : {}", LoggingHelper.createHexStringFromIntArray(data));
    ...
    public int createCheckSum(int[] frame) {
        ...
        LOG.debug("CHECKSUM : {}", LoggingHelper.createHexStringFromIntArray(new int[]{checksum}));
    ...
    private int putDataInMessage(int[] bytes, int next) {
        ...
        if (lastPos >= message.length && checkSumPos < bytes.length) {
            ...
            if (verifyCheckSum(bytes[checkSumPos])) {
                serialDataHandler.addMessageToReceivingQueue(message);
        ...
        while (next < bytes.length) {
            ...
            if (messageCreating) {
                ...
                switch (RkZWaveFrameMarker.getByCode(bytes[next])) {
                    ...
                    case ACK:
                        serialDataHandler.receiveAcknowledge();
                        next++;
                        break;

                    // node mot acknowledge our command
                    case NAC:
                        serialDataHandler.receiveNotAcknowledge();
                        next++;
                        break;

                    // node request for cancel
                    case CAN:
                        serialDataHandler.receiveCancel();
                        next++;
                        break;
        ...
        public void serialEvent(SerialPortEvent event) {
            ...
            if (event.isRXCHAR() && event.getEventValue() > 0) {
                ...
                try {
                    ...
                    LOG.debug("ANSWER  : {}", LoggingHelper.createHexStringFromIntArray(bytes));
